...
int main(int argc, char *argv[]) //argv[] ={id,mq1,mq3,ref_string}
{
	...
	while (cpg < no_of_pages)
	{
		...
		{
			printf("Process: Frame number from MMU received for process %d and pageno %d: %d\n" , id,pg_no[cpg], mmu_recv.frameno);
			cpg++;
		}
		else if (mmu_recv.frameno == -1) //here cpg will not be incremented
		{
			printf("Process: Page fault occured for process %d and pageno %d\n", id, pg_no[cpg]);
			
		}
		else if (mmu_recv.frameno == -2)
		{
			printf("Process: Invalid page reference for process %d and pageno %d\n", id, pg_no[cpg]) ;
			exit(1);
		}
	}
	printf("Process: Process %d Terminated successfly\n", id);
	mmu_send.pageno = -9;
	mmu_send.id = id;
	mmu_send.mtype = TOMMU;
	send_message_mmu(mq3, &mmu_send);
